...
3. 如果使用了 API 模式，数据仅使用您本地生成的匿名 Device-ID 保存在您自己的 SQLite 或 Cloudflare KV 中。`
    ...
    },
    {
      id: 'faq_6',
      category: '定时调度与排队机制',
      question: '订阅定时更新与定时测速是如何并发协同的？',
      answer: `SubPulse 内置了双路高精度后台心跳调度引擎：
1. <b>独立周期控制</b>：订阅更新与测速周期可分别独立设置（如订阅每 60 分钟拉取一次，节点测速每 30 分钟测一次），且均默认开启，支持单独关闭。
2. <b>全并发执行</b>：订阅拉取支持多订阅源并发并行拉取；测速支持 15~50 线程高并发探针与 12 并发 IP 回溯。
3. <b>排队防冲突机制</b>：如果测速任务触发时订阅正在拉取更新，测速任务会自动挂起排队，等待订阅拉取与去重计算完成后，立即自动对最新节点库进行测速，确保测速数据始终对应最新节点。`
    }
  ];
